...
def hex_to_cartesian(hex_posisiton):
        # Calculating the position in the new base
        x = math.sqrt(3.0) * hex_posisiton[0] + math.sqrt(3.0) / 2.0 * hex_posisiton[1]
        y = 3.0 / 2.0 * hex_posisiton[1]
...
def cartesian_to_hex(p):
    size = (1, 1)
    pt = (p[0] / size[0], p[1] / size[1])
    q = math.sqrt(3.0) / 3.0 * pt[0] + -1.0 / 3.0 * pt[1]
    r = 2.0 / 3.0 * pt[1]
    return (q, r)
...
def cube_distance(a, b):
    return max(math.fabs(a[0] - b[0]), math.fabs(a[1]- b[1]), math.fabs(a[2] - b[2]))

def axial_distance(a, b):
    c = axial_to_cube(a)
    d = axial_to_cube(b)
    return max(math.fabs(c[0] - d[0]), math.fabs(c[1] - d[1]), math.fabs(c[2] - d[2]))
...
def cube_round(cube):
    ...
    x_diff = math.fabs(rx - cube[0])
    y_diff = math.fabs(ry - cube[1])
    z_diff = math.fabs(rz - cube[2])
...
def cube_linedraw(a, b):
    ...
    for i in range(N+1):
        # print("lerp", cube_lerp(a, b, 1.0/N * i))
        temp = cube_round(cube_lerp(a, b, 1.0/N * i))
        results.append(temp)
        # print(temp)
    return results
...
def axial_linedraw(a, b):
    ...
    cube_result = cube_linedraw(c, d)
    axial_result = []
    for cube in cube_result:
        axial_result.append(cube_to_axial(cube))
    return axial_result

def cube_range(center, N:int):
    results = []
...
def cube_ring(center, radius):
    ...
    cube = center + cube_scale(cube_direction(4), radius)
    for i in range(6):
...
def hex_reachable(start, movement, blocked):
    ...
    for k in range(1, movement+1):
        ...
        for hex in fringes[k-1]:
            for dir in range(6):
                ...
                if not neighbor in visited and not neighbor in blocked:
